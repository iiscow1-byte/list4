...
const sort = ref<typeof SORTS[number]['value']>('submitted_desc')
...
const activeFilterCount = computed(() => {
  ...
  if (sort.value !== 'submitted_desc') n++
  return n
})

const items = ref<OpenVerRow[]>([])
const total = ref(0)
...
function buildQuery() {
  ...
  return {
    ...
    sort: sort.value !== 'submitted_desc' ? sort.value : undefined,
  }
}
...
async function loadMore() {
  ...
  loading.value = true
  try {
    const res = await $fetch<{ total: number; items: OpenVerRow[] }>('/api/open-verifications/levels', {
      query: buildQuery(),
    })
  ...
  <aside class="flex flex-col min-h-0 border-r border-zinc-800 bg-zinc-950">
    <div class="p-3 border-b border-zinc-800 shrink-0">
      <div class="flex items-center gap-2 mb-3 px-1">
        <span class="text-xs uppercase tracking-widest text-violet-300 font-semibold">Open verifications</span>
        <span class="text-[10px] text-zinc-500 normal-case tracking-normal">— unverified</span>
      </div>
