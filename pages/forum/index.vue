 ...
 * The forum.
 *
 * The site had two places to write and neither was a place to *start* a
 * conversation: comments belong to one level, progress posts belong to one
 * attempt. "Which of these should I grind next" has no level to hang off, and
 * "here's my route for the wave at 74%" is about a level but is not a comment
 * on it.
 *
 * A thread can name a level, and that optional link is what keeps this from
 * being a generic message board bolted onto a demon list.
 */
useHead({ title: 'Forum — All Levels List' })
...
/**
 * `?level_id=` narrows the board to one level's threads.
 *
 * How a level's own page links here — "all threads about this level" — so the
 * link has somewhere to go that isn't the whole forum. Read once rather than
 * kept in sync with the URL: it is an entry point, and the filters below are
 * for what you do after arriving.
 */
const route = useRoute()
const levelFilter = ref<number | null>(
  Number.isInteger(Number(route.query.level_id)) && Number(route.query.level_id) > 0
    ? Number(route.query.level_id)
    : null,
)
 ...
 * Starting a thread.
 *
 * On this page, in a panel, rather than behind a route of its own: writing a
 * post is the reason somebody came here, and making it a navigation away means
 * losing the list of what has already been said — which is where half of new
 * threads should start ("somebody already asked this").
 */
...
watch(levelQuery, (q) => {
  ...
  levelTimer = setTimeout(async () => {
    try {
      const res = await $fetch<{ items: { id: number; position: number; sheet_placement: number | null; name: string }[] }>(
...
async function submit() {
  ...
  try {
    ...
    await router.push(`/forum/${res.id}`)
  } catch (e: any) {
...
<template>
  <div class="container-tight max-w-5xl py-8 space-y-5">
    <header class="flex flex-wrap items-end justify-between gap-x-6 gap-y-3">
      <div class="min-w-0">
        ...
        <h1 class="mt-1 text-2xl sm:text-3xl font-bold tracking-tight text-zinc-50">Forum</h1>
        <p class="mt-1 text-sm text-zinc-500 max-w-2xl">
          ...
          show up on the level's own page.
        </p>
      </div>
      <button
        v-if="me"
      ...
      <NuxtLink v-else to="/login" class="btn btn-md btn-ghost shrink-0 hover:border-accent/60 hover:text-accent">
        Log in to post
      </NuxtLink>
    </header>
