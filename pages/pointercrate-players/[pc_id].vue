...
const pcId = computed(() => String(route.params.pc_id))
...
const { data, error } = await useFetch<{
  player: { name: string; country: string | null; subdivision: string | null; total_points: number; pack_points: number; extremes: number; hardest: string | null; rank: number | null; banned: boolean; skill_points: number; tier: string | null }
...
}>(() => `/api/pointercrate-players/${pcId.value}`, { watch: [pcId] })
...
<template>
  <div class="container-tight py-8 max-w-5xl">
    <div v-if="error" class="text-sm text-zinc-500">Pointercrate player not found.</div>
    <div v-else-if="data && !data.claimedBy" class="grid lg:grid-cols-[240px_minmax(0,1fr)] gap-6">
      ...
      <main class="space-y-6 min-w-0">
        <header class="flex items-start gap-4 flex-wrap">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span class="text-[10px] uppercase tracking-widest px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400">Pointercrate</span>
              <span
        ...
        <div class="rounded-md border border-amber-900/40 bg-amber-950/20 p-4 text-sm text-amber-200">
          This Pointercrate player has not claimed their account yet.
          <span class="text-amber-200/70 block text-xs mt-1">
            Stats below are mirrored from the Pointercrate list. If this is you,
        ...
        <section class="rounded-md border border-zinc-800 bg-zinc-950/60 p-4">
          ...
          <dl class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <div>
              <dt class="text-[10px] uppercase tracking-wider text-zinc-500">PC score</dt>
            ...
            <div>
              <dt class="text-[10px] uppercase tracking-wider text-zinc-500">PC rank</dt>
          ...
          <ul class="divide-y divide-zinc-900 text-sm">
            ...
            >
              <span
                ...
                :class="l.is_legacy ? 'bg-zinc-900 text-zinc-500' : 'bg-zinc-800 text-zinc-400'"
              >{{ l.is_legacy ? 'LEG' : `PC#${l.pc_position}` }}</span>
              ...
              <a v-if="l.video" :href="l.video" target="_blank" rel="noopener" class="text-accent hover:underline text-xs">video ↗</a>
            </li>
          </ul>
        </section>
      </main>
