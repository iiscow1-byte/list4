 ...
 *
 * A level counts as a challenge when any of three things is true:
 *
 *  1. its `placement_source` is one of the challenge lists (see
 *     `utils/challenge-sources.ts`),
 *  2. it is explicitly pinned with `rated = 'Challenge'`, or
 *  3. Geometry Dash itself describes it as one: unrated, zero score, Tiny or
 *     Short, and tiered on this list at Tier 1 or above.
 *
 * …and `not_challenge` is not set. That column is the override for a level the
 * third rule catches by accident, and it beats all three — an inference with no
 * way to be told it is wrong leaves a level stuck on a list it doesn't belong
 * on, which is exactly what the "unmark as challenge" button exists to fix.
 *
...
export function isChallengeSql(level = '', cache = 'c'): string {
  ...
  COALESCE(
    (
      ${challengeSourceSqlExpr(`${l}placement_source`)}
