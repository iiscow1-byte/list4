...
export const SITE_UPDATES: SiteUpdate[] = [
  {
    ...
    changes: [
      'Permanent links to levels. `alllevelslist.com/<level id>` opens whatever level has that Geometry Dash ID, wherever it currently sits. Every link anyone has shared until now pointed at a placement, and placements move — after a month of movements the link means a different level. There is a copy button next to the level ID. The same ID legitimately appears twice on the list sometimes, and in that case the link asks which you meant rather than guessing.',
      'The site speaks nine languages. English, Spanish, Portuguese, French, German, Russian, Polish, Chinese and Japanese, picked from the theme menu — and guessed from your browser the first time. The site\'s own wording only: level names, comments, bios and lists stay exactly as their authors wrote them.',
      'The challenge badge on a level opens the challenge list. It said "Challenge #12" and gave you no way to see what #12 was counted against; the challenge list existed, but only behind a menu, under a name that didn\'t obviously mean the same thing. Lists are linkable now in general — the address carries which one you are reading, so it survives clicking a level and can be shared.',
      'Levels removed from the list appear in the changelog. A removal was the one change to the list that could never show up in the list\'s own record of changes — the level\'s whole history was deleted along with it, so a level in the top 50 one day left no trace of having existed the next.',
      'Custom lists can be told what not to show. Tiers, difficulties and links back to the ALL are all assertions, and a list that isn\'t a demon list shouldn\'t be made to make them. There are four presets — standard, dense, gallery and standalone — and every switch stays editable afterwards.',
      'Custom lists handle long names. Level names run long and often differ only at the end, so a truncated panel could show two rows that were different levels and identical text. A list can now wrap names onto a second line or shrink them to fit, instead.',
      'The report button is in the same place everywhere, and always visible. It sat in four different positions across five surfaces, and on comments it was invisible until you happened to hover over it — which is no gesture at all on a phone. Its confirmation is a toast now, rather than a sentence that appeared inside the row and shoved everything sideways for six seconds.',
      'Fixed the movement arrows in the challenge changelog, which were measuring the main list. A challenge that slides four places down the main list past four ordinary levels has not moved on the challenge list at all, so the arrow beside `Ch. #14 → Ch. #14` was reporting a move that hadn\'t happened — and could point the wrong way outright.',
      'Fixed the profile header on a phone, where the name and the chips under it climbed up over the banner image and collided with the avatar.',
      adm('Undo, on the activity log. Role changes and challenge marks can be reversed from the entry that recorded them. The log stays append-only — undoing writes a second entry rather than erasing the first — and refuses when the change has already been superseded by a later one, or when the role is ranked above your own.'),
      adm('Removing a level asks for a reason and records it, along with the level\'s name, placement and tier. Once the row is gone that entry is the only account of what was taken off and why.'),
    ],
  },
  {
    version: '1.24.0',
    ...
    tags: ['UI', 'Mobile'],
    changes: [
      'The list works on a phone. Every list on the site — the main one, void, awaiting, open verifications and custom lists — was a fixed three-column layout with no breakpoint anywhere in it, so at 390px wide the level list was a 78px column of clipped single letters and the level itself got less than a paperback\'s width.',
      'The panels either side are drawers now, opened from a bar at the top. They keep their place while they are closed — your search, your scroll position, the pages you have loaded — because it is the same panel moved rather than a second copy.',
      'Custom lists were worse than the main list, not better: below a certain width their level list and all their records were simply hidden, with nothing to bring them back. On a phone you could not move between the levels of a list at all.',
      'The bottom of every list page was unreachable on a phone. The layout asked for the full height of the window, and a mobile browser reports that as the height with its address bar hidden — so roughly 80px of every panel sat permanently underneath it.',
      'Tapping a search box no longer zooms the page in and leaves it there. iOS zooms any field with text under 16px, and every field on this site was 14px, so one tap on the list\'s search left the whole site scaled up and scrolled sideways with every subsequent tap landing somewhere other than where it looked.',
      adm('The admin panel\'s tab bar is four menus instead of eleven loose controls, grouped by what you came to do rather than by which table the data is in.'),
      adm('List helper can actually be assigned. The role existed, the endpoint rejected it, and the accounts page never offered it.'),
      adm('Comments are recorded in the activity log, including their text when they are deleted. A deleted comment used to vanish entirely, leaving any report about it pointing at nothing.'),
    ],
  },
  {
    version: '1.23.0',
    ...
    changes: [
      'Making an account means proving an email address. A link arrives, and until it is followed the account can read the site but not comment, submit or send friend requests. An account that costs an inbox instead of a POST request is a different economic proposition for somebody making a hundred of them.',
      'A captcha on signing up and logging in, so the two doors that create accounts and guess passwords cannot be hammered by a script.',
      'Comments are rate limited — a handful a minute and a few dozen an hour — and posting the same comment twice within five minutes is treated as the double-clicked form it almost always is, rather than as a second thought.',
      'Fixed a way to run scripts on other people\'s browsers through an uploaded avatar. The image was trusted to be the kind of file it claimed to be; it is now checked against its actual contents, and the browser is told not to second-guess us either.',
      'Fixed an open redirect on the login page, which let a link that genuinely started with this site\'s address land you somewhere else entirely after signing in.',
      adm('Security headers across the site: a content security policy, frame-ancestors, nosniff and HSTS. There were none.'),
    ],
  },
  {
    version: '1.22.0',
    date: '2026-08-12',
    title: 'A role for the list, and a record of who did what',
    tags: ['Moderation', 'Accounts', 'Profiles', 'Fixes'],
    changes: [
      'Reporting. Accounts, comments, custom lists and levels can all be reported, with the reasons that fit each — including that a level is impossible, or that it should come off the list. Reporting a staff member for misusing their role goes to the site admins only, and never to the person it names.',
      'A "list helper" role: someone who can place levels and accept level and record submissions, and who asks rather than acts for anything that moves a level between rankings. It is not an admin role and carries nothing beyond that.',
      'Your bio, in its own editor, and the old About panel gone — it repeated what the header above it already said.',
      'A toggle for podium view on the leaderboards.',
      'Fixed the list appearing to freeze — the address bar would change while the page underneath stayed exactly where it was. Moving between menus through the home page asked the router to start a second navigation before the first had finished, and the two waited on each other.',
      'The credits name Claude AI, and The Shitty List, whose template this site started from.',
      adm('An activity log: one entry per action, who did it, and when — placements, roles, reports, comments and removals, filterable by area and searchable by name.'),
      adm('A "helper move requests" queue for the requests list helpers file.'),
      adm('Statistics can no longer be inflated by refreshing the page.'),
    ],
  },
  {
    version: '1.21.0',
